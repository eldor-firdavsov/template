...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { full_name, phone, telegram_user_id } = body as {
    full_name?: string;
    phone?: string;
  ...
  try {
    ...
    if (findErr) {
      console.error("Client search error:", findErr);
    }
    ...
    if (createErr || !newClient) {
      ...
      return res.status(500).json({ error: "Failed to create client profile", details: createErr?.message });
    }

    return res.json({ client_id: newClient.id, is_new: true });
  } catch (err: any) {
    console.error("Ensure client exception:", err);
